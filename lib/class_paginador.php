...
class pages{
	...
	public function registrosTotaleslistaImpresoras($connect, $area, $marca, $modelo, $tipo, $consumible){
		$sql = "SELECT COUNT(*) AS contar FROM impresoras WHERE area LIKE '%$area%' AND marca LIKE '%$marca%' AND modelo LIKE '%$modelo%' AND tipo LIKE '%$tipo%' AND consumible LIKE '%$consumible%'";
		$query = mysqli_query($connect, $sql);
		$row = mysqli_fetch_array($query);
		return $row['contar'];
	}
