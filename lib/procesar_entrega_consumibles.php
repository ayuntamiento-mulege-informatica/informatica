...
if (isset($_POST['accion'])) {
  ...
  if (!empty($_FILES['evidencia']['tmp_name'])) {
    if ($_FILES['evidencia']['type'] == 'image/jpeg') {
      if (is_uploaded_file($_FILES['evidencia']['tmp_name'])){
        ...
        header('location: /nuevo_registro_consumibles');
      }
    }
    else {
  ...
  $_SESSION['msg'] = $entrega_consumibles -> nuevaEntregaConsumibles($connect, $_POST['id'], $_POST['fecha_cambio'], $_POST['area'], $_POST['impresora'], $_POST['tipo'], $_POST['especificaciones'], $_POST['cantidad'], $_POST['recibe'], $evidencia);
